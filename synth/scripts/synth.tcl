...
# Need term -auto_map_names to keep track of name mapping between saif and netlist
if { $saifpower == 1 } {
    read_saif -input vcd/fma16.saif -auto_map_names -instance_name tb/dut -verbose   
}

puts $power_default_toggle_rate
puts $power_default_static_probability
...
redirect $filename { report_power -analysis_effort high -hierarchy -levels 99 -net -cell}
...
# Writes name map to file.ptpx for using saif file in PrimeTime
set filename "${my_toplevel}.ptpx"
saif_map -type ptpx -write_map $filename
